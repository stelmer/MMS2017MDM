...
$global:scriptVersion = "1.8 - 03.31.2017"
...
function CheckCommercialId
{
    ...
    Catch
    {
        Log "CommercialID mentioned in RunConfig.bat should be a GUID. It currently set to '$commercialIDValue'" "Error" "48" "CheckCommercialId" $_.Exception.HResult $_.Exception.Message
        Log "Script finished with error(s)" "Failure" "$global:errorCode" "ScriptEnd"
        [System.Environment]::Exit($global:errorCode)
    }
...
function CheckAppraiserKB
{
    ...
    Try
    {
        ...
        if($global:operatingSystemName.ToLower().Contains("windows 7") -or $global:operatingSystemName.ToLower().Contains("windows 8.1"))
        {
            ...
            if($result -eq $null -or $result -eq [string]::Empty)
            {
                Log "KB3150513 is not installed. Please install via http://www.catalog.update.microsoft.com/Search.aspx?q=KB3150513" "Error" "18" "CheckAppraiserKB"
                Log "Script finished with error(s)" "Failure" "$global:errorCode" "ScriptEnd"
                [System.Environment]::Exit($global:errorCode)
            }  
        }
...
function CheckProxySettings
{
    ...
    Try
    {
        ...
        $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings"
        $path2 = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings"

        Log "Registry properties at $path1 :"
        $winInetSettings1 = Get-ItemProperty -Path $path1
        foreach($setting1 in $winInetSettings1)
        {
            Log "$setting1"
        }

        Log "Registry properties at $path2 :"
        $winInetSettings2 = Get-ItemProperty -Path $path2
        foreach($setting2 in $winInetSettings2)
        {
            Log "$setting2"
        }
...
function CheckUserProxy
{
    Try
    {
        ...
        {
            $diagtrackVersion=[System.Diagnostics.FileVersionInfo]::GetVersionInfo("$global:windir\System32\diagtrack.dll").FileVersion.Split('.')[0..2] -join ''
